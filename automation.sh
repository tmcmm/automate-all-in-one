...
VERSION_EXIST=$(az aks get-versions -l $AKS_RG_LOCATION -ojson --query orchestrators[*].orchestratorVersion | jq -r ".[]" | grep $AKS_VERSION; echo $?)
...
options=("Azure Cluster" "Kubenet Cluster" "Private Cluster" "Linux DNS VM" "Chamar Jorge Aguas" "Queixar-se ao Shiva" "Destruir Tudo" "Quit")
